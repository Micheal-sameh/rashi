                ...
                <div class="card shadow-sm h-100">
                    <div class="card-body d-flex align-items-center">
                        <div class="me-3">
                            @if ($user->hasMedia('profile_images'))
                                <img src="{{ $user->getFirstMediaUrl('profile_images') }}"
                                     alt="{{ $user->name }}" width="60" height="60" class="rounded-circle shadow-sm zoomable-image"
                                     onclick="openPopup(this.src)" style="cursor:pointer;">
                            @else
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <small class="text-muted">{{ __('messages.points') }}: {{ $user->points ?? 0 }}</small>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer bg-transparent border-0 p-2 d-flex justify-content-middle">
                        <a href="{{ route('users.show', $user->id) }}"
                           class="btn btn-sm btn-primary shadow-sm"
                           title="{{ __('messages.view') }}">
                            <i class="fa fa-eye"></i>
